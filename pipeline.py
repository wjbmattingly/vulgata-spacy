...
order = st.multiselect("Which Component(s) would you like to use?", ["Annoy Matcher"])
for o in order:
    if o == "Annoy Matcher":
        cols = st.columns(2)
        annoy_max = cols[0].slider("Set Annoy Max Distance", .01, 1.0)
        annoy_num = cols[1].slider("Select Number of Results", 1, 5)
        annoy_style = st.selectbox("Select Method of Annoy", ["Found Quotes", "All Sentences"])
    elif o == "Fuzzy Matcher":
        fuzzy_max = st.slider("Set Annoy Max Distance", .01, 1.0)
text = st.text_area("Paste Text Here")
if len(text) > 0:
    nlp = Vulgate("models/vulgate_pipeline")
    doc = nlp.create_doc(text)
    for o in order:
        if o == "Annoy Matcher":
            if annoy_style=="All Sentences":
                style="sent"
            else:
                style="ent"
            nlp.annoy_matcher(style=style, max_distance=annoy_max, num_hits=annoy_num)
        if o == "Fuzzy Matcher":
            pass
    nlp.visualize_doc()
